...
| 模型                          | CMMLU     | 训练细节                                                  |
|-----------------------------|-----------|-------------------------------------------------------|
| **firefly-baichuan2-13b**   | **56.83** | 4\*V100，QLoRA，指令微调(100万条)                             |
| chinese-alpaca-2-13b        | 45.17     | 48\*A40，LoRA，词表扩充 + 增量预训练(120GB) + 指令微调(500万条)               |
| openbuddy-llama2-13b-v8.1   | 41.66     | 全量参数训练，词表扩充 + 指令微调                                    |
| belle-llama2-13b            | 41.57     | 8\*A100，全量参数训练，指令微调(40万条)                             |
| chinese-alpaca-2-7b         | 40.86     | 48\*A40，LoRA，词表扩充 + 增量预训练(120GB) + 指令微调(500万条)        |
| ziya-llama-13b*             | 39.9      | 160\*A100，全量参数训练，词表扩充 + 增量预训练(110B token) + 指令微调 + RLHF |
| chinese-alpaca-plus-13b*    | 39.9      | 48\*A40，LoRA，词表扩充 + 增量预训练(120GB ) + 指令微调(430万条)       |
| **firefly-llama2-13b-chat** | **39.47** | 4\*V100，QLoRA，词表扩充 + 增量预训练(22GB) + 指令微调(200万条)        |
| flagalpha-llama2-13b-chat   | 39.20     | LoRA，指令微调                                             |
| llama-2-13b-chat            | 38.65     | 全量参数训练，预训练 + 指令微调(27540条) + RLHF(全流程为英文)              |
| **firefly-llama2-7b-chat**  | **34.03** | 4\*V100，QLoRA，词表扩充 + 增量预训练(22GB) + 指令微调(200万条)        |
| llama-2-7b-chat             | 33.76     | 全量参数训练，预训练 + 指令微调(27540条) + RLHF(全流程为英文)              |
| flagalpha-llama2-7b-chat    | 32.61     | LoRA，指令微调                                             |
| chinese-alpaca-plus-7b*     | 32.6      | 48\*A40，LoRA，词表扩充 + 增量预训练(120GB) + 指令微调(430万条)        |
| yayi-13b-llama2             | 30.73     | 指令微调(百万条)                                             |
| yayi-7b-llama2              | 30.47     | 指令微调(百万条)                                             |
...
- 我们的模型在CMMLU上的指标与chinese-alpaca-2也存在一定的差距。这一现象很大程度与增量预训练数据量和数据分布相关，我们的增量预训练数据仅为22GB，增量预训练不够充分，且大部分为新闻语料，对于CMMLU能力的提升有限。
...
同为基于LLaMA2进行汉化的模型，我们首先对Firefly-LLaMA2-13B-Chat和Linly-LLaMA2-13B进行了人工测评，从评测结果来看，我们的模型存在非常大的优势。
