...
在Open LLM Leaderboard上超越Ziya，在CMMLU上比Ziya略低0.43分。在人工测评中，我们的模型以33.08%获胜、60.77%平局、6.15%失败的成绩，超越Linly。
...
| 模型                          | CMMLU     | 训练细节                                        | 预训练数据                                           | 指令数据                                               |
|-----------------------------|-----------|---------------------------------------------|-------------------------------------------------|----|
| **firefly-baichuan2-13b**   | **56.83** | 4\*V100，QLoRA，指令微调                          | 无                                           | 100万条                                              |
| chinese-alpaca-2-13b        | 45.17     | 48\*A40，LoRA，词表扩充 + 增量预训练 + 指令微调            | 120GB                                           | 500万条                                              |
| openbuddy-llama2-13b-v8.1   | 41.66     | 全量参数训练，词表扩充 + 指令微调                          | 无                                               | 未知                                                 |
| belle-llama2-13b            | 41.57     | 8\*A100，全量参数训练，指令微调                         | 无                                               | 40万条                                               |
| chinese-alpaca-2-7b         | 40.86     | 48\*A40，LoRA，词表扩充 + 增量预训练 + 指令微调            | 120GB                                           | 500万条                                              |
| ziya-llama-13b*             | 39.9      | 160\*A100，全量参数训练，词表扩充 + 增量预训练 + 指令微调 + RLHF | 110B token                                      | 未知                                                 |
| chinese-alpaca-plus-13b*    | 39.9      | 48\*A40，LoRA，词表扩充 + 增量预训练 + 指令微调            | 120GB                                                | 430万条                                                   |
| **firefly-llama2-13b-chat** | **39.47** | 4\*V100，QLoRA，词表扩充 + 增量预训练 + 指令微调           | 22GB                                            | 200万条                                              |
| flagalpha-llama2-13b-chat   | 39.20     | LoRA，指令微调                                   | 无                                               | 未知                                                 |
| llama-2-13b-chat            | 38.65     | 全量参数训练，预训练 + 指令微调 + RLHF(全流程为英文)            | 英文数据                                            | 27540条                                             |
| **firefly-llama2-7b-chat**  | **34.03** | 4\*V100，QLoRA，词表扩充 + 增量预训练 + 指令微调           | 22GB                                            | 200万条                                              |
| llama-2-7b-chat             | 33.76     | 全量参数训练，预训练 + 指令微调 + RLHF(全流程为英文)            | 英文数据                                            | 27540条                                             |
| flagalpha-llama2-7b-chat    | 32.61     | LoRA，指令微调                                   | 无                                               | 未知                                                 |
| chinese-alpaca-plus-7b*     | 32.6      | 48\*A40，LoRA，词表扩充 + 增量预训练 + 指令微调            | 120GB                                                | 430万条                                                   |
| yayi-13b-llama2             | 30.73     | 指令微调                                        | 无                                               | 百万                                                 |
| yayi-7b-llama2              | 30.47     | 指令微调                                        | 无                                               | 百万                                                 |
| linly-llama2-7b             | 28.68     | 32\*A100，全量参数训练，词表扩充 + 混合训练                 | 量级未知                                            | 量级未知   |
| linly-llama2-13b            | 26.32     | 32\*A100，全量参数训练，词表扩充 + 混合训练                 | 量级未知  | 量级未知 |
