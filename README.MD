...
## 项目简介
本项目与[Firefly](https://github.com/yangjianxin1/Firefly)一脉相承，专注于**低资源增量预训练**，既支持对Baichuan2、Qwen、InternLM等原生中文模型进行增量预训练，也可
对LLaMA2、Falcon等英文模型进行中文词表扩充，然后进行增量预训练。
...
**更重要的是，在整个增量预训练和指令微调阶段，我们最多仅使用了4\*V100的GPU，训练更加低资源高效。相较于Ziya的160\*A100，Linly的32\*A100，Chinese-LLaMA-Alpaca的48\*A40，Firefly-LLaMA-Chiense所使用的训练资源少得多。**
...
Conclusion：我们的模型保留了llama2模型优秀的英文能力。


### CMMLU榜单

| 模型                          | CMMLU     | 训练资源       | 训练方法   | 训练流程                  | 预训练数据                                           | 指令数据                                               |
|-----------------------------|-----------|------------|--------|----------------|-------------------------------------------------|----|
| **firefly-baichuan2-13b**   | **56.83** | 4 x V100   | QLoRA  |  指令微调   | 无                                           | 100万条                                              |
| chinese-alpaca-2-13b        | 45.17     | 48 x A40   | LoRA   | 词表扩充 + 增量预训练 + 指令微调   | 120GB                                           | 500万条                                              |
| openbuddy-llama2-13b-v8.1   | 41.66     | -          | 全量参数训练 | 词表扩充 + 指令微调           | 无                                               | 未知                                                 |
| belle-llama2-13b            | 41.57     | 8 x A100   | 全量参数训练 | 指令微调                  | 无                                               | 40万条                                               |
| chinese-alpaca-2-7b         | 40.86     | 48 x A40   | LoRA   | 词表扩充 + 增量预训练 + 指令微调   | 120GB                                           | 500万条                                              |
| ziya-llama-13b*             | 39.9      | 160 x A100 | 全量参数训练 | 词表扩充 + 增量预训练 + 指令微调 + RLHF | 110B token                                      | 未知                                                 |
| chinese-alpaca-plus-13b*    | 39.9      | 48 x A40         | LoRA   | 词表扩充 + 增量预训练 + 指令微调                      | 120GB                                                | 430万条                                                   |
| **firefly-llama2-13b-chat** | **39.47** | 4 x V100   | QLoRA  | 词表扩充 + 增量预训练 + 指令微调   | 22GB                                            | 200万条                                              |
| flagalpha-llama2-13b-chat   | 39.20     | -          | LoRA   | 指令微调                  | 无                                               | 未知                                                 |
| llama-2-13b-chat            | 38.65     | -          | 全量参数训练 | 预训练 + 指令微调 + RLHF(全流程为英文) | 英文数据                                            | 27540条                                             |
| **firefly-llama2-7b-chat**  | **34.03** | 4 x V100   | QLoRA  | 词表扩充 + 增量预训练 + 指令微调   | 22GB                                            | 200万条                                              |
| llama-2-7b-chat             | 33.76     | -          | 全量参数训练 | 预训练 + 指令微调 + RLHF(全流程为英文) | 英文数据                                            | 27540条                                             |
| flagalpha-llama2-7b-chat    | 32.61     | -          | LoRA   | 指令微调                  | 无                                               | 未知                                                 |
| chinese-alpaca-plus-7b*     | 32.6      | 48 x A40         | LoRA   | 词表扩充 + 增量预训练 + 指令微调                      | 120GB                                                | 430万条                                                   |
| yayi-13b-llama2             | 30.73     | -          | 未知     | 指令微调                  | 无                                               | 百万                                                 |
| yayi-7b-llama2              | 30.47     | -          | 未知     | 指令微调                  | 无                                               | 百万                                                 |
| linly-llama2-7b             | 28.68     | 32 x A100  | 全量参数训练 | 词表扩充 + 混合训练           | 量级未知                                            | 量级未知   |
| linly-llama2-13b            | 26.32     | 32x A100   | 全量参数训练 | 词表扩充 + 混合训练           | 量级未知  | 量级未知 |

我们统一采用OpenCompass工具来离线评测CMMLU，其中*表示结果来源于OpenCompass官方榜单或者由模型作者自测的分数。

Conclusions:
- 与llama-2-chat相比，我们的模型在中文方面的能力具有一定的提升。
- 对于中文词表扩充模型而言，我们的模型大幅领先全量训练的linly，与全量训练的ziya基本持平，但与chinese-alpaca-2模型还存在一定的差距。
...
- 我们的模型在CMMLU上的指标与chinese-alpaca-2也存在一定的差距。这一现象很大程度与增量预训练数据量和数据分布相关，我们的增量预训练数据仅为22GB（且未完全利用，详情见训练细节），增量预训练不够充分，且大部分为新闻语料，对于CMMLU能力的提升有限。
...
同为基于LLaMA2进行汉化的模型，我们首先对Firefly-LLaMA2-13B-Chat和Linly-LLaMA2-13B进行了人工测评，从评测结果来看，我们的模型存在较大的优势。
...
我们将增量预训练数据集命名为firefly-pretrain-dataset，主要包含CLUE、ThucNews、CNews、COIG、维基百科等开源数据集，以及我们收集的古诗词、散文、文言文等。数据分布如下图：
...
由于训练资源的原因，在增量预训练阶段，我们未充分使用22GB的预训练数据，关于增量预训练的详情介绍后续会更新。
